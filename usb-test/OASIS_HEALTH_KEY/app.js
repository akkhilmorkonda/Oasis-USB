...
(function () {
  ...
  function renderEncounters(encounters) {
    if (!encounters || encounters.length === 0) {
      $('section-encounters').hidden = true;
      return;
    }
    $('section-encounters').hidden = false;
    const html = encounters
      .map((e) => {
        ...
        return `
          <div class="card">
            ...
            <div class="card__row"><span class="card__label">Type</span><span class="card__value">${escapeHtml(type)}</span></div>
          </div>
        `;
      })
      .join('');
    $('encounters-list').innerHTML = html;
  }
  ...
  function renderObservations(observations) {
    if (!observations || observations.length === 0) {
      $('section-observations').hidden = true;
      return;
    }
    $('section-observations').hidden = false;
    const html = observations
      .map((obs) => {
        const { label, value, unit, date } = getObservationDisplay(obs);
        return `
          <div class="observation-card">
            ...
            <div class="observation-card__date">${escapeHtml(formatDateTime(date))}</div>
          </div>
        `;
      })
      .join('');
    $('observations-list').innerHTML = html;
  }

  function renderConditions(conditions) {
    if (!conditions || conditions.length === 0) {
      $('section-conditions').hidden = true;
      return;
    }
    $('section-conditions').hidden = false;
    const html = conditions
      ...
      .join('');
    $('conditions-list').innerHTML = html;
  }
  ...
  function renderMedia(docs, mediaList) {
    const items = [...(docs || []), ...(mediaList || [])];
    if (items.length === 0) {
      $('section-media').hidden = true;
      return;
    }
    $('section-media').hidden = false;
